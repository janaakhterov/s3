...
// DeleteBucketPolicy requset Headers, this list *MUST* be in
...
pub struct DeleteBucketPolicy<T: AsRef<str>> {
    /// Bucket name from which to Delete the policy.
    pub bucket: T,
}

impl<T: AsRef<str>> DeleteBucketPolicy<T> {
    /// Create a new DeleteBucketPolicy
    pub fn new(bucket: T) -> Self {
        DeleteBucketPolicy { bucket }
    }
}

impl<T: AsRef<str>> AwsRequest for DeleteBucketPolicy<T> {
    type Response = ();
